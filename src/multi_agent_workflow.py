...
class OutputAgent:
    ...
    def _load_rag_context(self) -> str:
        """Retrieve FAISS context (PubMed + GeneCards + arXiv)."""
        try:
            ...
            query = f"Function, disease mechanisms, and clinical phenotypes of {self.gene_name}"
            docs = retriever.get_relevant_documents(query)
            print(f"  [OutputAgent] Retrieved {len(docs)} documents from FAISS")
            return "\n\n---\n\n".join(
                f"[{doc.metadata.get('source', 'Unknown')}] {doc.page_content}"
                for doc in docs
            )
        except Exception as e:
            print(f"  [OutputAgent] RAG context error: {e}")
            return ""
